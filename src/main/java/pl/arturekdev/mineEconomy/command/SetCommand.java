...
public class SetCommand extends SubCommand {
    ...
    public SetCommand(String name, List<String> aliases, String permission, EcoMessages ecoMessages, EconomyService economyService) {
    ...
    public void handleCommand(CommandSender sender, String[] arguments) {
        ...
        if (arguments.length != 2) {
            MessageUtil.sendMessage(sender, ecoMessages.setCommandUsage());
            return;
        }
        ...
        MessageUtil.sendMessage(sender, ecoMessages.successSetMoney().replace("%player%", user.getName()).replace("%value%", String.valueOf(value)));
    }
}
